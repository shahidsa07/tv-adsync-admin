...
export async function GET(req: NextRequest) {
    const socket = (req.socket as unknown as Socket & { server: any }).server;

    if (socket.ws) {
        console.log("WebSocket server already attached.");
    } else {
        console.log("Attaching WebSocket server.");
        socket.ws = true; // Mark that we've attached the listener
    ...
    // This is the tricky part. API Routes in Next.js expect a Response object.
    // We're returning a 426 Upgrade Required, which is semantically correct
    // for a WebSocket upgrade request that isn't a WebSocket request.
    // The client-side WebSocket will handle this.
